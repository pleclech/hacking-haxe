...
module IdentifierType = struct
	type t =
		...
		| ITGlobal of module_type * string * Typedef.t
...
type shared_display_information = {
	...
	mutable type_hints : (pos,Typedef.t) Hashtbl.t;
...
type context = {
	...
	mutable main_class : Typedef.path option;
	...
	cached_macros : (path * string,((string * bool * t) list * t * tclass * Typedef.tclass_field)) Hashtbl.t;
	mutable stored_typed_exprs : (int, texpr) PMap.t;
	(* output *)
	mutable file : string;
	mutable flash_version : float;
	mutable features : (string,bool) Hashtbl.t;
	mutable modules : Typedef.module_def list;
	mutable main : Typedef.texpr option;
	mutable types : Typedef.module_type list;
